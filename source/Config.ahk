...
Config_init() {
	...
	Config_readinMem		:= False						; If true, the current memory usage is read in and displayed in the status bar.
	...
	Config_readinInterval	:= 60000						; Time in milliseconds after which the above status values are refreshed.
...
Config_readinAny() {										; Add information to the variable "text" in this function to display it in the status bar.
	Global Config_readinCpu, Config_readinMem, Config_readinDate
	
	text := ""
	If Config_readinCpu
		text .= " CPU: " Bar_getSystemTimes() "% "
	If Config_readinMem
		text .= " Mem: " Bar_getMemoryUsage() "% "
	If (Config_readinDate Or Config_readinMem) And Config_readinCpu
		text .= "|"
...
Config_saveSessionWindows() {
	Local m, v, text
    FileDelete, oldWindows
	...
	Loop, % Manager_monitorCount {
		...
		Loop, % Config_viewCount {
            ...
            text .= "View_#" m "_#" v "_wndIds=" wndIds "`n"
        }
    }
    
    FileAppend, %text%, oldWindows
}
