...
Bar_powershellGet(cmd, filename) {
    runwait,%comspec% /c powershell.exe -c %cmd% >%filename%,,hide
...
Bar_getMemoryUsage() {
    Static total_mem_cmd = "(Get-WmiObject -Class Win32_OperatingSystem).TotalVisibleMemorySize"
    Static free_mem_cmd = "(Get-WmiObject -Class Win32_OperatingSystem).FreePhysicalMemory"

    total_mem := Bar_powershellGet(total_mem_cmd, "TotalVisibleMemorySize.tmp")
    free_mem  := Bar_powershellGet(free_mem_cmd, "FreePhysicalMemory.tmp")

    perc_used := Round(( total_mem - free_mem ) / total_mem * 100)
